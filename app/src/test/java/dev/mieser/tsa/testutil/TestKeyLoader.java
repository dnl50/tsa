...
/**
 * Util class for loading self-signed certificates and their corresponding private keys which can be used in tests.
 */
...
public class TestKeyLoader {
    ...
    private static <T> T loadResourceFromClasspath(String path, ThrowingFunction<InputStream, T> streamConsumer) {
        try (InputStream resourceStream = TestKeyLoader.class.getResourceAsStream(path)) {
